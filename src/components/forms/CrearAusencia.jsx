...
import styles from "./crearAusencia.module.css";
...
const CrearAusencia = () => {
  ...
  const showSuccessToast = () => {
    Toast.fire({
      icon: "success",
      title: "Ausencia creada exitosamente",
    });
  };
  ...
  const handleSubmit = async (event) => {
    ...
    // Validación de la fecha (comprobar si es una fecha válida)
    ...
    const nuevaAusencia = {
      empleadoID,
      fecha: fechaObjeto, // Enviar como objeto Date
      tipo,
      motivo,
      estado,
    };
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/ausencias/crear",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setError(errorData.message || "Error al crear la ausencia.");
        Swal.fire({
          icon: "error",
          title: "Error al crear",
          text: "Error al crear la ausencia: " + errorData.message,
        });
      }
    } catch (error) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error al crear",
        text: "Error al crear la ausencia.",
      });
    }
  };
